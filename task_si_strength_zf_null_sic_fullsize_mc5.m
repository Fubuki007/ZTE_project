% =========================================================================
% task_si_strength_zf_null_sic_fullsize_mc5.m
%   完整尺寸 SI 强度扫描：beta_SI = [1, 10, 100, 1000, 10000]
%   Ns=12672, L=256, Mrx=64, SNR=0 dB, MC=5
%   串行运行（省内存），每个 MC 重新生成发射波形
% =========================================================================
function task_si_strength_zf_null_sic_fullsize_mc5()
clear; close all; clc; warning('off','all');
t_all = tic;

log_path = fullfile(pwd, 'task_si_strength_zf_null_sic_fullsize_mc5.log');
...
fprintf('=== Full-size SI strength sweep (MC=5, serial) ===\n');
...
for mi = 1:n_methods
    ...
    for bi = 1:n_beta
        ...
        % checkpoint
        save(fullfile(pwd, 'task_si_strength_zf_null_sic_fullsize_mc5_checkpoint.mat'), ...
            'beta_list','methods','sic_flags','method_labels','n_mc', ...
            'rmse_R','rmse_theta','rmse_v','mi','bi');
    end
end

fprintf('\n=== Full-size SI strength results (MC=5) ===\n');
for bi=1:n_beta
...
save(fullfile(pwd, 'task_si_strength_zf_null_sic_fullsize_mc5.mat'), ...
    'beta_list','methods','sic_flags','method_labels','n_mc', ...
