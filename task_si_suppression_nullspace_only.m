% =========================================================================
% task_si_suppression_nullspace_only.m
%   单方法脚本：Null-space
...
%   每个 MC 重新生成发射波形。
% =========================================================================
function task_si_suppression_nullspace_only()
t_all = tic;
warning('off','all');

log_path = fullfile(pwd, 'task_si_suppression_nullspace_only.log');
if isfile(log_path), delete(log_path); end
diary(log_path); diary on;

method      = 'nullspace';
sic_flag    = false;
method_label = 'Null-space';
...
save(fullfile(pwd, 'task_si_suppression_nullspace_only.mat'), ...
...
for fi = 1:3
    ...
    base = fullfile(fig_dir, sprintf('fig_si_suppression_nullspace_only_%s', fnames{fi}));
