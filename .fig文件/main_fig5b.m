...
%%test parameters
Ntest =5e2;
Ns_range_dB=15:2.5:32.5;
Ns_range=round( 10.^(0.1.*Ns_range_dB)  );
...
SNRa=zeros(2,length(Ns_range));
SNRn=zeros(7,length(Ns_range));

SNRa_sum=zeros(2,length(Ns_range));
SNRn_sum=zeros(7,length(Ns_range));
for i_HChange=1:Ntest
    ...
    user_d=tpara.R;
    for i_Ns=1:length(Ns_range)
        para.Ns=Ns_range(i_Ns);
        tpara.N_fft_d=a_zero_padding*para.Ns;
        xianshi=['外层循环改变次数：',num2str(i_HChange),'Ns:',num2str(para.Ns)];
        disp(xianshi);
        %%视距信道
        ...
        for mod_type=1:modnum
            ...
            D_r=D_r_nfree+Noise;
            [SNRa(mod_type,i_Ns),SNRn((mod_type-1)*3+1,i_Ns)]=SNR_of_my_proposed(para,tpara,D_r,X0);
            %% -------------- Comparisons ------------------
            %compa1 [29]
            [SNRn((mod_type-1)*3+2,i_Ns)]=SNR_of_compAlg_29(para,tpara,D_r);
            %compa2 [28]
            [SNRn((mod_type-1)*3+3,i_Ns)]=SNR_of_compAlg_28(para,tpara,D_r);
        end
        %LFMCW
        [SNRn(7,i_Ns)]=SNR_of_LFMCW(H,para,tpara,SNR);
    end
...
figure;
semilogx(Ns_range, 10*log10(fSNRn(7,:)),'o-k','linewidth',1.5); %%LFMCW黑色
hold on;
plot(Ns_range, 10*log10(fSNRn(3,:)),'v-','color','#096C28','linewidth',1.5); %%[28]绿色
plot(Ns_range, 10*log10(fSNRn(2,:)),'d-','color','#7D2E8D','linewidth',1.5);%%[29]紫色
plot(Ns_range, 10*log10(fSNRn(1,:)),'x-r','linewidth',1.5); %%提出 红色

xlabel('The number of subcarriers');
