% =========================================================================
% task_si_strength_zf_null_sic_fullsize_mc10.m
%   完整尺寸 SI 强度扫描：beta_SI = [1, 10, 100, 1000, 10000]
%   Ns=12672, L=256, Mrx=64, SNR=0 dB, MC=10
%   串行运行（省内存），每个 MC 重新生成发射波形
% =========================================================================
function task_si_strength_zf_null_sic_fullsize_mc10()
clear; close all; clc; warning('off','all');
t_all = tic;

log_path = fullfile(pwd, 'task_si_strength_zf_null_sic_fullsize_mc10.log');
...
fprintf('=== Full-size SI strength sweep (MC=10, serial) ===\n');
...
for mi = 1:n_methods
    ...
    for bi = 1:n_beta
        ...
        % checkpoint
        save(fullfile(pwd, 'task_si_strength_zf_null_sic_fullsize_mc10_checkpoint.mat'), ...
...
save(fullfile(pwd, 'task_si_strength_zf_null_sic_fullsize_mc10.mat'), ...
    'beta_list','methods','sic_flags','method_labels','n_mc', ...
