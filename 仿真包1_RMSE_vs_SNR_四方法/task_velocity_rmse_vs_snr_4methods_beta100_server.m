% =========================================================================
% task_velocity_rmse_vs_snr_4methods_beta100_server.m
% -------------------------------------------------------------------------
% 速度 RMSE vs SNR — 四方法对比 (服务器全量版, 严格串行)
% -------------------------------------------------------------------------
% 四根曲线 (enable_SI=true, beta_SI=100, 非理想 SIC=40 dB):
...
%
% 横坐标: SNR = -45:5:20 dB (14 点);  纵坐标: 速度 RMSE (m/s), 对数轴.
% 速度口径: evaluate_estimation 返回的 rmse_v, 对两个目标统计.
%
% 全尺寸参数:
...
%
% ★ 内存安全设计 (16 GB 机器, 串行跑, 时间可以长但不能中途爆内存):
%   - 全程无 parfor, 启动时自动关闭残留并行池 (全尺寸 rx_cube ~3.3 GB/次,
%     并行 4 worker 曾直接爆内存, 见历史日志)
%   - 每个方法的发射波形只生成一次 (X_tx ~0.83 GB), 全部 SNR/MC 复用
%   - 每次 MC 结束后立即 clear rx_cube 等大变量
%   - 峰值内存 ~4.5 GB (rx_cube 3.3 + X_tx 0.83 + FFT 中间量), 安全
%   - 每个 MC 完成后保存 checkpoint (文件很小), 断电/崩溃后重跑自动续跑
%
% 规模与耗时: 4 方法 x 14 SNR x 60 MC = 3360 次全尺寸估计,
%   按历史日志 (MC10 全尺寸 63.9 min/150 次) 推算约 18~24 小时.
%
% 运行 (在项目根目录, 建议 nohup / tmux 挂后台):
%   matlab -batch "task_velocity_rmse_vs_snr_4methods_beta100_server"         % 全量
%   matlab -batch "task_velocity_rmse_vs_snr_4methods_beta100_server(true)"   % 冒烟(上机自检)
%
% 输出 (均在项目根目录):
%   task_velocity_rmse_vs_snr_4methods_beta100_server.mat / .csv / .log
%   task_velocity_rmse_vs_snr_4methods_beta100_server_checkpoint.mat
%   fig/fig_velocity_rmse_vs_snr_4methods_beta100_server.fig/.png/.eps
% =========================================================================
function task_velocity_rmse_vs_snr_4methods_beta100_server(smoke_test)
...
log_path = fullfile(script_dir, ['task_velocity_rmse_vs_snr_4methods_beta100' mode_tag '.log']);
if isfile(log_path), delete(log_path); end
diary(log_path); diary on;

% ==================== 1. 配置 ==============================================
beta_SI_val = 100;                 % SI 幅度相对目标回波的倍数
sic_cancel_db = 40;                % 数字 SIC 功率抑制量
sic_residual_amplitude = 10^(-sic_cancel_db/20); % 40 dB -> 1% 幅度残余
sim_model_version = 'partial_sic_40dB_common_random_v1';
L_val       = 256;                 % OFDM 符号数
Mx_val      = 8;  My_val = 8;      % 接收阵列 8x8 -> Mrx=64
K_stream    = 2;                   % 通信用户/空间流数

if smoke_test
    snr_list = [-45, -30, -15, 0, 15];   % 冒烟: 5 点 (上机自检)
    n_mc     = 1;
else
...
baseParams.sic_residual_amplitude = sic_residual_amplitude;
baseParams.K          = L_val;
...
% ==================== 4. 预生成随机种子 (可复现) =========================
rng(20260801);
% 四种方法共用每个 SNR/MC 的噪声，实现配对 Monte Carlo 公平比较。
rng_seeds = randi(2^31-1, n_snr, n_mc);

% ==================== 5. 打印参数核对 ======================================
n_trials = n_methods * n_snr * n_mc;
fprintf('═══════════════════════════════════════════════════════════════\n');
fprintf('  速度 RMSE vs SNR — 四方法对比 (%s)\n', mode_label);
fprintf('  开始时间: %s\n', char(datetime('now')));
fprintf('═══════════════════════════════════════════════════════════════\n');
...
fprintf('  Digital SIC: %g dB power suppression, residual amplitude=%.4g\n', ...
    sic_cancel_db, sic_residual_amplitude);
fprintf('  SNR: [%d, %d] dB 共 %d 点, MC=%d\n', ...
    snr_list(1), snr_list(end), n_snr, n_mc);
fprintf('  总估计次数: %d 方法 x %d SNR x %d MC = %d\n', ...
    n_methods, n_snr, n_mc, n_trials);
fprintf('  运行模式: 严格串行 (无 parfor), 峰值内存 ~4.5 GB\n');
...
ckpt_path = fullfile(script_dir, ...
    ['task_velocity_rmse_vs_snr_4methods_beta100' mode_tag '_checkpoint.mat']);
if isfile(ckpt_path)
    tmp = load(ckpt_path);
    ok = isfield(tmp,'rmse_R') && isfield(tmp,'rmse_theta') && ...
         isfield(tmp,'rmse_v') && isfield(tmp,'done_mask') && ...
         isfield(tmp,'snr_list') && isfield(tmp,'beta_SI_val') && ...
         isfield(tmp,'method_tags') && isfield(tmp,'sim_model_version') && ...
         isfield(tmp,'sic_residual_amplitude') && ...
         isequal(size(tmp.rmse_R), [n_methods n_snr n_mc]) && ...
         isequal(size(tmp.rmse_theta), [n_methods n_snr n_mc]) && ...
         isequal(size(tmp.rmse_v), [n_methods n_snr n_mc]) && ...
         isequal(tmp.snr_list(:), snr_list(:)) && ...
         tmp.beta_SI_val == beta_SI_val && ...
         strcmp(tmp.sim_model_version, sim_model_version) && ...
         tmp.sic_residual_amplitude == sic_residual_amplitude && ...
...
for mi = 1:n_methods
    ...
    % Reset before every method so all precoders use identical QAM symbols.
    rng(20260730);
    ...
    fprintf('  si_leak=%.4g, comm_err=%.3g\n', ...
        tx.precoder_info.si_leak_avg, tx.precoder_info.comm_err_avg);
    sic_amp = 1;
    if m.use_sic, sic_amp = sic_residual_amplitude; end
    residual_si_metric = beta_SI_val^2 * ...
        tx.precoder_info.si_leak_avg * sic_amp^2;
    fprintf('  residual_SI_metric=%.4g (lower is better)\n', ...
        residual_si_metric);
    clear tx;
    ...
    for si = 1:n_snr
        ...
        for mc_i = 1:n_mc
            ...
            try
                ...
                if isfield(cmp, 'rmse_v') && ~isnan(cmp.rmse_v)
            ...
            save(ckpt_path, 'rmse_R','rmse_theta','rmse_v','done_mask', ...
                'snr_list','beta_SI_val','method_tags','sim_model_version', ...
                'sic_residual_amplitude','mi','si');
        end

        el = toc(t_blk);
        n_done_now = sum(done_mask(:)) - n_done0;
        if n_done_now > 0
            sec_per_trial = (toc(t_all) / 60) / n_done_now;      % 分钟/次
            eta_h = (n_trials - n_done_now) * sec_per_trial / 60;
            eta_str = sprintf('整体已跑 %.0f min, 剩余 ETA ~%.1f h', ...
                toc(t_all)/60, eta_h);
        else
            eta_str = '';
        end
        fprintf(' | med v=%.4f m/s (NaN %d/%d) | %s | %s\n', ...
            median(rmse_v(mi, si, :), 'omitnan'), ...
            sum(isnan(rmse_v(mi, si, :))), n_mc, ...
...
fprintf('\n═══════════════════════════════════════════════════════════════\n');
fprintf('  速度 RMSE (m/s) 摘要 (median over %d MC)\n', n_mc);
fprintf('═══════════════════════════════════════════════════════════════\n');
fprintf('%8s', 'SNR(dB)');
...
for si = 1:n_snr
    ...
    for mi = 1:n_methods, fprintf('  %16.4f', rmse_v_med(mi, si)); end
    fprintf('\n');
end
...
results.sic_cancel_db  = sic_cancel_db;
results.sic_residual_amplitude = sic_residual_amplitude;
results.sim_model_version = sim_model_version;
results.primary_metric = 'rmse_v';
results.primary_unit   = 'm/s';
...
results.params_summary = struct( ...
    ...
    'K_stream', K_stream, 'num_targets', baseParams.num_targets, ...
...
mat_path = fullfile(script_dir, ...
    ['task_velocity_rmse_vs_snr_4methods_beta100' mode_tag '.mat']);
save(mat_path, '-struct', 'results');
fprintf('\n数据已保存: %s\n', mat_path);

% CSV
csv_path = fullfile(script_dir, ...
    ['task_velocity_rmse_vs_snr_4methods_beta100' mode_tag '.csv']);
fid = fopen(csv_path, 'w');
fprintf(fid, 'SNR_dB,%s\n', strjoin({methods.tag}, ','));
for si = 1:n_snr
    fprintf(fid, '%+d', snr_list(si));
    for mi = 1:n_methods
        fprintf(fid, ',%.6e', rmse_v_med(mi, si));
    end
    fprintf(fid, '\n');
end
fclose(fid);
...
for mi = 1:n_methods
    h(mi) = semilogy(snr_list, max(rmse_v_med(mi, :), 1e-12), '-', ...
...
ylabel('Velocity RMSE (m/s)', 'FontName', 'Times New Roman', 'FontSize', 11);
title(sprintf('Velocity RMSE vs SNR (\\beta_{SI} = %g, MC = %d)', ...
...
base_path = fullfile(fig_dir, ...
    ['fig_velocity_rmse_vs_snr_4methods_beta100' mode_tag]);
...
if ~isunix
    ...
    if isfile(toast_script)
        try
            cmd = sprintf('python "%s" "速度RMSE-SNR四方法(服务器)" "%d方法x%dSNRx%dMC, %.1fh"', ...
                toast_script, n_methods, n_snr, n_mc, toc(t_all)/3600);
            system(cmd);
        catch
        end
    end
