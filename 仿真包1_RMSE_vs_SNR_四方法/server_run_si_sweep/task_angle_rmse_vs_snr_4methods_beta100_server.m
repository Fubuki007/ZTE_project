% =========================================================================
% task_angle_rmse_vs_snr_4methods_beta100_server.m
% -------------------------------------------------------------------------
% 角度 RMSE vs SNR — 四方法对比 (服务器全量版, 严格串行)
% -------------------------------------------------------------------------
...
%
% 横坐标: SNR = -45:5:20 dB (14 点);  纵坐标: 角度 RMSE (deg), 对数轴.
% 角度口径: evaluate_estimation 返回的 rmse_theta (俯仰角), 与项目既有
% Angle RMSE 脚本一致; 不与方位角 rmse_phi 合并.
%
% 全尺寸参数:
...
%   matlab -batch "task_angle_rmse_vs_snr_4methods_beta100_server"         % 全量
%   matlab -batch "task_angle_rmse_vs_snr_4methods_beta100_server(true)"   % 冒烟(上机自检)
%
% 输出 (均在项目根目录):
%   task_angle_rmse_vs_snr_4methods_beta100_server.mat / .csv / .log
%   task_angle_rmse_vs_snr_4methods_beta100_server_checkpoint.mat
%   fig/fig_angle_rmse_vs_snr_4methods_beta100_server.fig/.png/.eps
% =========================================================================
function task_angle_rmse_vs_snr_4methods_beta100_server(smoke_test)
...
log_path = fullfile(script_dir, ['task_angle_rmse_vs_snr_4methods_beta100' mode_tag '.log']);
...
fprintf('═══════════════════════════════════════════════════════════════\n');
fprintf('  角度 RMSE vs SNR — 四方法对比 (%s)\n', mode_label);
fprintf('  开始时间: %s\n', char(datetime('now')));
fprintf('═══════════════════════════════════════════════════════════════\n');
...
ckpt_path = fullfile(script_dir, ...
    ['task_angle_rmse_vs_snr_4methods_beta100' mode_tag '_checkpoint.mat']);
if isfile(ckpt_path)
    tmp = load(ckpt_path);
...
for mi = 1:n_methods
    ...
    for si = 1:n_snr
        ...
        fprintf(' | med theta=%.4f deg (NaN %d/%d) | %s | %s\n', ...
            median(rmse_theta(mi, si, :), 'omitnan'), ...
            sum(isnan(rmse_theta(mi, si, :))), n_mc, ...
...
fprintf('\n═══════════════════════════════════════════════════════════════\n');
fprintf('  角度 RMSE (deg) 摘要 (median over %d MC)\n', n_mc);
fprintf('═══════════════════════════════════════════════════════════════\n');
fprintf('%8s', 'SNR(dB)');
...
for si = 1:n_snr
    ...
    for mi = 1:n_methods, fprintf('  %16.4f', rmse_theta_med(mi, si)); end
    fprintf('\n');
end
...
results.primary_metric = 'rmse_theta';
results.primary_unit   = 'deg';
...
mat_path = fullfile(script_dir, ...
    ['task_angle_rmse_vs_snr_4methods_beta100' mode_tag '.mat']);
save(mat_path, '-struct', 'results');
fprintf('\n数据已保存: %s\n', mat_path);

% CSV
csv_path = fullfile(script_dir, ...
    ['task_angle_rmse_vs_snr_4methods_beta100' mode_tag '.csv']);
fid = fopen(csv_path, 'w');
fprintf(fid, 'SNR_dB,%s\n', strjoin({methods.tag}, ','));
for si = 1:n_snr
    fprintf(fid, '%+d', snr_list(si));
    for mi = 1:n_methods
        fprintf(fid, ',%.6e', rmse_theta_med(mi, si));
    end
    fprintf(fid, '\n');
end
fclose(fid);
...
for mi = 1:n_methods
    h(mi) = semilogy(snr_list, max(rmse_theta_med(mi, :), 1e-12), '-', ...
...
title(sprintf('Angle RMSE vs SNR (\\beta_{SI} = %g, MC = %d)', ...
...
base_path = fullfile(fig_dir, ...
    ['fig_angle_rmse_vs_snr_4methods_beta100' mode_tag]);
...
if ~isunix
    ...
    if isfile(toast_script)
        try
            cmd = sprintf('python "%s" "角度RMSE-SNR四方法(服务器)" "%d方法x%dSNRx%dMC, %.1fh"', ...
