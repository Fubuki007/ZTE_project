% =========================================================================
% task_range_rmse_vs_snr_4methods_beta100_server.m
% -------------------------------------------------------------------------
% 距离 RMSE vs SNR — 四方法对比 (服务器全量版, 严格串行)
% -------------------------------------------------------------------------
...
%
% 规模与耗时: 4 方法 x 14 SNR x 50 MC = 2800 次全尺寸估计,
%   按历史日志 (MC10 全尺寸 63.9 min/150 次) 推算约 15~20 小时.
%
% 运行 (在项目根目录, 建议 nohup / tmux 挂后台):
%   matlab -batch "task_range_rmse_vs_snr_4methods_beta100_server"         % 全量
%   matlab -batch "task_range_rmse_vs_snr_4methods_beta100_server(true)"   % 冒烟(上机自检)
%
% 输出 (均在项目根目录):
%   task_range_rmse_vs_snr_4methods_beta100_server.mat / .csv / .log
%   task_range_rmse_vs_snr_4methods_beta100_server_checkpoint.mat
%   fig/fig_range_rmse_vs_snr_4methods_beta100_server.fig/.png/.eps
% =========================================================================
function task_range_rmse_vs_snr_4methods_beta100_server(smoke_test)
...
if ~isunix
    ...
    if isfile(toast_script)
        try
            cmd = sprintf('python "%s" "RMSE-SNR四方法(服务器)" "%d方法x%dSNRx%dMC, %.1fh"', ...
