...
%
%   SNR = -60:5:25, MC = 20, L = 256, Mrx = 64, beta_SI = 100
%   每个 MC 重新生成发射波形。
% =========================================================================
function task_si_suppression_nullspace_sic_only_mc20()
t_all = tic;
warning('off','all');

log_path = fullfile(pwd, 'task_si_suppression_nullspace_sic_only_mc20.log');
...
save(fullfile(pwd, 'task_si_suppression_nullspace_sic_only_mc20.mat'), ...
...
titles  = {'Angle RMSE vs SNR (L = 256, MC = 20, beta=100)', ...
           'Range RMSE vs SNR (L = 256, MC = 20, beta=100)', ...
           'Velocity RMSE vs SNR (L = 256, MC = 20, beta=100)'};
...
for fi = 1:3
    ...
    base = fullfile(fig_dir, sprintf('fig_si_suppression_nullspace_sic_only_mc20_%s', fnames{fi}));
