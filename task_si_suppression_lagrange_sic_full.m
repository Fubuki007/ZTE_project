% =========================================================================
% task_si_suppression_lagrange_sic_full.m
%   服务器完整版：Angle / Range / Velocity RMSE vs SNR
%   三条曲线：
%     1. ZF
%     2. Lagrange
%     3. Lagrange + digital SIC (LS 估计 SI 信道，参考 SIC.m)
%
%   参数与之前 7 点图保持一致：
%     SNR = -60:5:30, MC = 100, L = 256, Mrx = 16, beta_SI = 10
%
%   运行：
%     matlab -batch "task_si_suppression_lagrange_sic_full"
%
%   输出：
%     task_si_suppression_lagrange_sic_full.mat
%     fig/fig_si_suppression_lagrange_sic_{angle,velocity,range}.fig/.png
% =========================================================================
function task_si_suppression_lagrange_sic_full()
t_all = tic;
warning('off','all');

% ==================== 0. Log ==============================================
log_path = fullfile(pwd, 'task_si_suppression_lagrange_sic_full.log');
if isfile(log_path), delete(log_path); end
diary(log_path); diary on;

fprintf('============================================================\n');
fprintf('  Lagringe+SIC validation (full server version)\n');
fprintf('  ZF / Lagrange / Lagrange + digital SIC\n');
fprintf('  Start: %s\n', char(datetime('now')));
fprintf('============================================================\n\n');
...
save(fullfile(pwd, 'task_si_suppression_lagrange_sic_full.mat'), ...
...
for fi = 1:3
    ...
    base_path = fullfile(fig_dir, sprintf('fig_si_suppression_lagrange_sic_%s', fnames{fi}));
