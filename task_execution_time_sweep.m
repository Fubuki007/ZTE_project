function task_execution_time_sweep()
...
log_path = fullfile(pwd, 'execution_time_sweep.log');
if isfile(log_path)
    delete(log_path);
end
diary(log_path);
...
fprintf('MC=%d, receiver=%dx%d, SNR=%g dB, SI=OFF\n', ...
...
result_path = fullfile(pwd, 'task_execution_time_sweep.mat');
save(result_path, 'Ns_list', 'L_list', 'fixed_L_for_Ns', ...
    'fixed_Ns_for_L', 'Mx_fixed', 'My_fixed', 'input_snr_db', ...
    'n_mc', 'seed_base', 'time_ns_all', 'time_L_all', ...
...
fprintf('Benchmark completed in %.1f min: %s\n', ...
    ...
    function save_checkpoint()
        ...
        save(fullfile(pwd, 'task_execution_time_sweep_checkpoint.mat'), ...
    ...
    function write_summary_tables()
        ...
        l_table = table(L_list(:), time_L_mean, time_L_median, ...
            ...
            'MedianSeconds', 'StdSeconds', 'MonteCarloRuns'});
        writetable(ns_table, fullfile(pwd, 'execution_time_vs_subcarriers.csv'));
        writetable(l_table, fullfile(pwd, 'execution_time_vs_cpi_length.csv'));
    end
    ...
    function create_and_save_figures()
        ...
        save_benchmark_figure(fig_ns, 'execution_time_vs_subcarriers', ...
            fig_dir, png_dir);

        fig_L = create_benchmark_figure(L_list, time_L_mean, ...
            'CPI length', 'Impact of CPI length', 2);
        save_benchmark_figure(fig_L, 'execution_time_vs_cpi_length', ...
            fig_dir, png_dir);
        close([fig_ns, fig_L]);
    end
end
...
loglog(ax, x, y, '-d', ...
    ...
    'MarkerSize', 5.2, ...
    'LineWidth', 1.35, ...
    'DisplayName', 'Proposed estimator');
