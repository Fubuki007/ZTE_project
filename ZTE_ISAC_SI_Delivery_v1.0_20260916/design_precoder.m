...
%   method 字符串:
%     'zf'         —— 传统 ZF, 不考虑自干扰
%     'nullspace'  —— 在通信零空间内减小 SI 泄漏（Tikhonov 正则化）
%     'lagrange'   —— SI 加权 ZF（正则化线性方程）
%
% 可选 opts:
%   opts.normalize (默认 true)  —— 是否对 W 做 Frobenius 归一
%   opts.eps_reg   (默认 1e-9)  —— 最小正则项, 防止奇异
%   opts.ns_lambda / opts.lg_lambda (默认 1e-3) —— 各方法正则强度
%
% 输出:
%   W           (Nt × K) 预编码矩阵
%   diag_info   struct: si_leak 为功率归一后 ||H_SI*W||_F^2;
%               comm_err 为功率归一前 ||H_c'*W-I||_F^2。两者不在同一尺度。
% =========================================================================
